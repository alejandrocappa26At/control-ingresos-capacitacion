    ...
    <div className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-line bg-surface-2 p-5 shadow-card transition-all duration-300 hover:-translate-y-0.5 hover:shadow-glow-rose">
      ...
      <div className="relative mb-3 flex items-center gap-2.5">
        ...
        <div className="min-w-0 leading-tight">
          <p className="truncate text-[13px] font-bold tracking-wide text-ink uppercase">Día de caída por sede</p>
          <p className="truncate text-xs text-ink-soft">Ranking premium por sede{subtitle ? ` · ${subtitle}` : ''}</p>
        </div>
      </div>

      {top && (
        <div className="relative mb-3 overflow-hidden rounded-2xl border border-rose-500/25 bg-gradient-to-br from-rose-500/[0.12] via-rose-900/[0.06] to-transparent p-3.5 shadow-[0_10px_36px_-14px_rgba(227,6,19,0.45)]">
          <div className="pointer-events-none absolute -top-10 -right-10 size-32 rounded-full bg-rose-500/20 blur-3xl" />
          <p className="relative text-[10px] font-black tracking-widest text-rose-300 uppercase">🔥 Sede con mayor impacto</p>
          <div className="relative mt-1 flex flex-wrap items-end justify-between gap-x-4 gap-y-1">
            ...
            <div className="flex items-center gap-3">
              <span className="rounded-full border border-rose-400/30 bg-rose-500/10 px-2 py-0.5 text-[11px] font-bold text-rose-200">
                Día {top.diaPromedio != null ? top.diaPromedio.toFixed(1) : '—'}
              </span>
              <span className="text-lg font-black tabular-nums text-rose-300">{formatNumber(top.total)} caídas</span>
            </div>
          </div>
        </div>
      )}
        ...
        {rows.map((r, i) => {
          ...
          return (
            <motion.div
              ...
              className="rounded-xl border border-white/5 bg-white/[0.03] p-2.5 pr-3 transition-colors duration-200 hover:border-rose-400/30 hover:bg-rose-500/[0.06]"
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex shrink-0 items-center gap-3">
                  <span className="flex items-center gap-1 rounded-full border border-rose-400/20 bg-rose-500/10 px-2 py-0.5 text-[11px] font-bold text-rose-200">
                    <CalendarDays className="size-3 text-rose-300" />
                    Día {r.diaPromedio != null ? r.diaPromedio.toFixed(1) : '—'}
                  </span>
                  <span className="font-black tabular-nums text-rose-300">{formatNumber(r.total)}</span>
                </div>
              </div>
              <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-white/5">
                <motion.div
                  ...
                  className="h-full rounded-full bg-gradient-to-r from-rose-700 via-rose-500 to-rose-400 shadow-[0_0_12px_rgba(244,63,94,0.55)]"
                />
              </div>
            </motion.div>
          );
      ...
      <p className="relative mt-3 text-center text-[11px] font-medium text-ink-soft">
        Día promedio de caída y total de caídas · {formatNumber(totalCaidas)} caídas en el periodo
      </p>
    </div>
  );
}