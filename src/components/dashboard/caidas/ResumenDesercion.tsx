...
import { TasaDesercionPremium } from './TasaDesercionPremium';
import { BajasCapacitacionPremium } from './BajasCapacitacionPremium';
import { MesMayorDesercionPremium } from './MesMayorDesercionPremium';
  ...
  return (
    <section className="mt-6 space-y-4">
      ...
      >
        ...
        <span className="inline-flex items-center gap-2 rounded-xl border border-red-500/25 bg-red-500/10 px-3 py-1.5 text-xs font-bold text-red-300 shadow-[0_0_16px_-6px_rgba(227,6,19,0.4)]">
          {formatNumber(data.totalDeserciones)} deserciones en el periodo
        </span>
      </motion.div>

      <div className="grid gap-4 lg:grid-cols-3">
        <TasaDesercionPremium data={data} />
        <BajasCapacitacionPremium data={data} />
        <MesMayorDesercionPremium data={data} />
      </div>
    </section>
  );
}
