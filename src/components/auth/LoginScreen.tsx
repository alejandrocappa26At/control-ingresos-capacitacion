...
function BrandMark({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' }) {
  const dims = size === 'sm' ? 'size-10 rounded-xl' : size === 'md' ? 'size-12 rounded-2xl' : 'size-14 rounded-2xl';
  const iconDims = size === 'sm' ? 'size-5' : size === 'md' ? 'size-6' : 'size-7';
  const badgeDims = size === 'sm' ? 'size-4 rounded-md' : size === 'md' ? 'size-5 rounded-md' : 'size-6 rounded-lg';
  const badgeIcon = size === 'sm' ? 'size-2.5' : size === 'md' ? 'size-3' : 'size-3.5';

  return (
    <div className={cn('relative flex shrink-0 items-center justify-center gradient-brand text-white shadow-glow', dims)}>
      <Monitor className={iconDims} />
      <div className={cn('absolute -right-1 -bottom-1 flex items-center justify-center bg-[#020617]', badgeDims)}>
        <TrendingUp className={cn('text-cyan-400', badgeIcon)} style={{ filter: 'drop-shadow(0 0 6px rgba(6,182,212,0.9))' }} />
      </div>
    </div>
  );
}
...
/* ------------------------------------------------------------------ */
/* Fondo dinámico: aurora + grilla + partículas + luz                 */
/* ------------------------------------------------------------------ */
...
function LoginBackground() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
      ...
      {/* Aurora blobs */}
      <div
        className="absolute -top-[20%] -left-[12%] size-[60vw] rounded-full opacity-70"
        style={{
          background: 'radial-gradient(circle at 30% 30%, rgba(99,102,241,0.32), transparent 62%)',
          filter: 'blur(90px)',
          animation: 'aurora-drift 26s ease-in-out infinite alternate',
        }}
      />
      <div
        className="absolute top-[8%] -right-[16%] size-[54vw] rounded-full opacity-60"
        style={{
          background: 'radial-gradient(circle at 60% 40%, rgba(139,92,246,0.28), transparent 60%)',
          filter: 'blur(100px)',
          animation: 'aurora-drift 32s ease-in-out infinite alternate-reverse',
        }}
      />
      <div
        className="absolute -bottom-[24%] left-[18%] size-[58vw] rounded-full opacity-60"
        style={{
          background: 'radial-gradient(circle at 40% 60%, rgba(6,182,212,0.24), transparent 62%)',
          filter: 'blur(110px)',
          animation: 'aurora-drift 38s ease-in-out infinite alternate',
        }}
      />

      {/* Riel de luz superior */}
      <div
        className="absolute top-0 left-1/2 h-[420px] w-[900px] -translate-x-1/2"
        style={{
          background: 'radial-gradient(ellipse at center, rgba(99,102,241,0.2), rgba(6,182,212,0.08) 45%, transparent 70%)',
          filter: 'blur(40px)',
...
const MOCK_STATS: Array<{ label: string; value: string; dot: string; tone: string }> = [
  { label: 'Ingresos', value: '3572', dot: 'bg-indigo-400', tone: 'text-indigo-300' },
  { label: 'Aprobados', value: '2795', dot: 'bg-emerald-400', tone: 'text-emerald-300' },
  { label: 'Caídas', value: '739', dot: 'bg-rose-400', tone: 'text-rose-300' },
  { label: 'En proceso', value: '38', dot: 'bg-amber-400', tone: 'text-amber-300' },
];
...
function SparklineArea() {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="h-24 w-full" preserveAspectRatio="none">
      ...
      />
      <circle cx={step * (points.length - 1)} cy={h - points[points.length - 1] * (h - 12) - 6} r="3.5" fill="#06b6d4" />
    </svg>
  );
}
...
function DashboardMock() {
  return (
    <div className="glass relative w-full rounded-2xl border border-white/10 p-5 shadow-[0_24px_60px_-18px_rgba(0,0,0,0.7)] backdrop-blur-xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">Panel ejecutivo</p>
          </div>
        </div>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="glass rounded-2xl border border-white/10 p-3.5 shadow-[0_16px_44px_-14px_rgba(0,0,0,0.7)] backdrop-blur-xl"
      >
        {children}
      </motion.div>
    </motion.div>
  ...
  return (
    <div className="relative hidden flex-col justify-center px-12 py-12 lg:flex xl:px-20">
      ...
      >
        <BrandMark size="md" />
        <div>
          <p className="text-sm font-bold tracking-tight text-white">Control de Ingresos</p>
          <p className="text-[11px] font-medium text-slate-400">y Capacitación</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <p className="mt-4 flex items-center gap-2 text-base font-medium text-slate-300 xl:text-lg">
      ...
      >
        <div className="absolute -top-10 -left-8 size-40 rounded-full bg-indigo-500/20 blur-3xl" />
        ...
        <FloatCard delay={0.55} floatDelay="0.6" className="-right-6 -top-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400">Índice de pase a operaciones</p>
            </div>
          </div>
        </FloatCard>
        ...
        <FloatCard delay={0.7} floatDelay="1.8" className="bottom-2 -left-8">
          <div className="flex items-center gap-2.5">
            <span className="flex size-8 items-center justify-center rounded-xl border border-cyan-400/30 bg-cyan-400/10 text-cyan-300">
              <CheckCircle2 className="size-4" />
            </span>
            <div>
              <p className="text-xs font-bold text-white">Sincronizado</p>
              <p className="text-[10px] text-slate-400">Última carga · hace 4 min</p>
            </div>
          </div>
        </FloatCard>
      </motion.div>
      ...
      <motion.p
        ...
        className="mt-10 flex items-center gap-2 text-[11px] font-medium text-slate-500"
      >
...
function Field({
  ...
  return (
    <div>
      <label htmlFor={id} className="text-[11px] font-bold tracking-wider text-slate-300 uppercase">
        {label}
      </label>
      <div className="group relative mt-1.5">
        <span
          className={cn(
            ...
            error ? 'text-rose-400' : value ? 'text-indigo-300' : 'text-slate-500 group-focus-within:text-indigo-300',
          )}
        >
          {icon}
        </span>
        <input
          ...
          className={cn(
            ...
            error
              ? 'border-rose-500/50 shadow-[0_0_0_4px_rgba(244,63,94,0.12),0_0_20px_-6px_rgba(244,63,94,0.45)]'
              : 'border-white/10 focus:border-indigo-400/60 focus:bg-white/[0.06] focus:shadow-[0_0_0_4px_rgba(99,102,241,0.14),0_0_24px_-8px_rgba(99,102,241,0.5)]',
          )}
        />
        {type === 'password' && (
          <button
            ...
            className="absolute top-1/2 right-3 -translate-y-1/2 rounded-lg p-1 text-slate-500 transition-colors duration-200 hover:text-slate-300"
          >
      ...
      <div className="relative z-10 mx-auto grid min-h-dvh max-w-[1500px] lg:grid-cols-[minmax(0,11fr)_minmax(0,9fr)]">
        ...
        <div className="flex items-center justify-center px-5 py-10 sm:px-10 lg:px-8 xl:px-16">
          ...
          >
            <motion.div
              animate={controls}
              className="relative rounded-3xl border border-white/[0.08] p-8 shadow-[0_40px_120px_-30px_rgba(0,0,0,0.85)] sm:p-10"
              style={{
            ...
            >
              {/* Halo decorativo del card */}
              <div
                className="pointer-events-none absolute -inset-px rounded-3xl opacity-60"
                style={{
                  background:
                    'linear-gradient(135deg, rgba(99,102,241,0.16), transparent 30%, transparent 70%, rgba(6,182,212,0.14))',
              ...
              <div className="flex items-center gap-3 lg:hidden">
                ...
                <div>
                  <p className="text-sm font-bold tracking-tight text-white">Control de Ingresos</p>
                  <p className="text-[11px] text-slate-400">y Capacitación</p>
                </div>
              </div>
              ...
              <div className="mt-6 lg:mt-0">
                <motion.h1
                  ...
                  className="text-2xl font-extrabold tracking-tight text-white xl:text-[28px]"
                >
                  Bienvenido
                </motion.h1>
                <motion.p
              ...
              <form onSubmit={handleSubmit} className="mt-8 space-y-3.5" noValidate>
                ...
                >
                  <motion.button
                    ...
                    className={cn(
                      'group relative flex w-full cursor-pointer items-center justify-center gap-2 overflow-hidden rounded-xl py-3.5 text-sm font-bold tracking-[0.06em] text-white uppercase shadow-[0_16px_44px_-12px_rgba(99,102,241,0.55)] transition-shadow duration-300',
                      'hover:shadow-[0_20px_60px_-10px_rgba(99,102,241,0.75)]',
                      submitting && 'cursor-wait',
                    )}
                    style={{
                      background: 'linear-gradient(120deg, #6366f1, #8b5cf6 35%, #06b6d4 70%, #6366f1)',
              ...
              <motion.div
                ...
                className="mt-7 flex items-center justify-center gap-1.5 border-t border-white/[0.07] pt-5 text-[11px] font-medium text-slate-500"
              >
            ...
            <div className="pointer-events-none mt-6 hidden items-center justify-center gap-6 text-slate-600 sm:flex" aria-hidden>
              <CheckCircle2 className="size-4" />
